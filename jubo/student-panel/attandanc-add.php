    ...
    if(empty($user_lat) || empty($user_lng)){
        ...
        if($is_in_range){
            ...
            if(mysqli_num_rows($check_duplicate) > 0){
                ...
                $insert = mysqli_query($con, "INSERT INTO attendance (student_id, branch_id, att_date, check_in, lat, lng, mp, yp) VALUES ('$student_id', '$branch_id', '$today', '$current_time', '$user_lat', '$user_lng', '$current_month', '$current_year')");
                if($insert){
...
if(isset($_POST['check_out_btn'])){
    ...
    if(empty($user_lat) || empty($user_lng)){
        ...
        if($is_in_range){
            $update = mysqli_query($con, "UPDATE attendance SET check_out='$current_time' WHERE student_id='$student_id' AND att_date='$today'");
            if($update){
