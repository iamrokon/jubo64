...
<div class="container-fluid">
  ...
  <table width="100%" class="table m-0 table-bordered" id="dataTables-example">
   ...
   $query = "SELECT distinct student_list.student_id, student_list.stu_name, student_list.gender, 
          student_list.father_name, student_list.contact, student_list.fiver, student_list.upwork,
          district.dist_name, jobs.job_name, 
          income_info.earning_dollar, income_info.earning_bd,
          (SELECT work_source.work_name 
           FROM income_info as ii 
           LEFT JOIN work_source ON work_source.ws_id = ii.work_source 
           WHERE ii.student_id = student_list.student_id 
          ...
          LEFT JOIN income_info ON income_info.student_id = student_list.student_id
          LEFT JOIN jobs ON jobs.j_id = income_info.job_id
          WHERE student_list.batch_id = 13
        ...
        while ($eqrow = mysqli_fetch_assoc($eq)) {
          ...
          <tr>
            ...
            <!-- <td><strong style="font-size: 22px;">৳</strong><?= number_format($eqrow['earning_bd'], 2); ?></td> -->
             <td><?= number_format(floatval($eqrow['earning_bd']), 2); ?></td>
