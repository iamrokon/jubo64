				...
				<div class="panel panel-default">
					<div class="panel-heading">
						...
						if ($_SERVER['REQUEST_METHOD'] === 'POST') {
							...
							$sql = $con->query("SELECT count(`stu_id`) as `total` FROM `student_reg_jubo_new` WHERE entry_date >= '$start_date' AND entry_date <= '$end_date' ");
							$row = $sql->fetch_assoc();
							$GT = $row['total'];
						}
						?>
						...
						<table width="100%" class="table table-bordered table-hover text-center" id="testTableCount">
							...
							<tbody id="tbody">
								...
								if ($_SERVER['REQUEST_METHOD'] === 'POST') {
									...
									$sql = "SELECT  a.district, b.dist_name, a.div_id,d.div_name, COUNT(a.stu_id) as total_students 
                    FROM student_reg_jubo_new a 
